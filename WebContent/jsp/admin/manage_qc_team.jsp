    ...
    if ("add".equals(action)) {
        ...
        if (vtype != null && fname != null && uname != null && pwd != null
            ...
            try (Connection conn = DBConnection.getConnection()) {
                // Skip duplicate check - handled by DB unique constraint
                int exists = 0;
                if (exists > 0) {
                    ...
                    PreparedStatement slotQ = conn.prepareStatement(
                        "SELECT COALESCE(MAX(slot_number),0)+1 AS ns FROM module_team_members WHERE module_role='qc' AND vehicle_type=?");
                    ...
                    PreparedStatement pu = conn.prepareStatement(
                        "INSERT INTO users (full_name, username, password, role, is_active) VALUES (?,?,?,'qc',1)");
                    ...
                    msg = "QC Team member " + fname.trim() + " added as Slot " + nextSlot + " for " + vtLabel;
                    msgType = "success";
                }
            } catch (Exception e) {
    ...
    if ("toggle".equals(action)) {
        ...
        if (didStr != null && curStr != null) {
            try (Connection conn = DBConnection.getConnection()) {
                ...
                msg = newVal == 1 ? "QC Team member activated." : "QC Team member deactivated.";
                msgType = "success";
            } catch (Exception e) {
                msg = "Error: " + e.getMessage();
                msgType = "error";
            ...
            try (Connection conn = DBConnection.getConnection()) {
                ...
                if (dr.next()) {
                    ...
                    ResultSet rem = conn.createStatement().executeQuery(
                        "SELECT id FROM module_team_members WHERE module_role='qc' AND vehicle_type='"
                    ...
                    PreparedStatement rreset = conn.prepareStatement(
                        "UPDATE module_round_robin SET last_slot_used=0 WHERE module_role='qc' AND vehicle_type=?");
                ...
                msg = "QC Team member removed from pool.";
                msgType = "success";
            } catch (Exception e) {
                msg = "Error: " + e.getMessage();
                msgType = "error";
...
<title>Manage QC Team — AutoProd</title>
...
.ifield:focus{border-color:#16a34a;}
...
.next-badge{background:#dcfce7;color:#15803d;font-size:.65rem;font-weight:800;padding:2px 7px;border-radius:10px;margin-left:6px;}
.slot-num{background:#22c55e;color:#fff;border-radius:50%;width:26px;height:26px;display:inline-flex;align-items:center;justify-content:center;font-size:.72rem;font-weight:800;}
</style>
</head>
<body>
...
<div class="sb">
    ...
    <a class="nl active" href="manage_designers.jsp"><i class="bi bi-shield-check"></i> Manage QC Team</a>
...
<div class="main">
    <div class="topbar">
        <div>
            <div style="font-family:'Syne',sans-serif;font-size:1.05rem;font-weight:800;color:var(--brand);">
                <i class="bi bi-shield-check"></i> Manage QC Team
            </div>
            <div style="font-size:.72rem;color:var(--muted);margin-top:1px;">
                Add QC Team members per vehicle category &mdash; Round-robin auto-assignment
            </div>
        </div>
        <a href="admin_dashboard.jsp?tab=qc_assign" class="sbtn" style="font-size:.8rem;padding:8px 16px;">
    ...
    <div class="pb">
        ...
        <!-- How It Works -->
        <div style="background:linear-gradient(135deg,#eff6ff,#f0fdf4);border:1.5px solid #15803d;border-radius:14px;padding:16px 20px;margin-bottom:24px;display:flex;align-items:flex-start;gap:14px;">
            <span style="font-size:2rem;">&#128260;</span>
            <div>
                <div style="font-weight:800;color:#16a34a;font-size:.95rem;margin-bottom:4px;">Round-Robin Auto Assignment — QC Team</div>
            ...
            for (String[] vt : vtypes) {
                ...
                try {
                    PreparedStatement dq = conn.prepareStatement(
                        ...
                        "WHERE vd.module_role='qc' AND vd.vehicle_type = ? ORDER BY vd.slot_number");
                ...
                try {
                    PreparedStatement rq = conn.prepareStatement(
                        "SELECT last_slot_used, total_assigned FROM module_round_robin WHERE module_role='qc' AND vehicle_type=?");
        ...
        <div class="card">
            ...
            <div class="ch">
                <div class="ct">
                    ...
                    <%= vtLabel %> QC Team
                    <span style="background:#22c55e;color:#fff;border-radius:20px;padding:2px 10px;font-size:.68rem;font-weight:800;margin-left:4px;">
            ...
            <div style="padding:18px 22px;display:flex;align-items:center;gap:12px;background:#fffde7;">
                ...
                <div>
                    <div style="font-weight:700;color:#b45309;font-size:.88rem;">No QC Team members added yet</div>
                    <div style="font-size:.77rem;color:#92400e;margin-top:2px;">
                        Use the form below to add the first <%= vtLabel %> QC Team member
                    </div>
                </div>
            </div>
            <% } else { %>
            <table class="dt">
                ...
                %>
                <tr style='<%= isNext ? "background:#f0fdf4;" : "" %>'>
                    <td>
                    ...
                    <td>
                        <div style="display:flex;gap:6px;">
                            ...
                            <form method="post" action="manage_qc_team.jsp" style="display:inline;">
            ...
            <!-- Add Designer Form -->
            <div class="add-form">
                <div style="font-size:.77rem;font-weight:800;color:#16a34a;text-transform:uppercase;letter-spacing:.5px;margin-bottom:12px;">
                    <i class="bi bi-person-plus-fill"></i> Add New <%= vtLabel %> QC Team Member &mdash; Will be Slot <%= totalDesigners + 1 %>
                </div>
                <form method="post" action="manage_qc_team.jsp">
                    ...
                    <div class="add-grid">
                        ...
                        <div>
                            <button type="submit" class="sbtn"
                                style="background:linear-gradient(135deg,#16a34a,#15803d);white-space:nowrap;padding:10px 16px;">
...
<div id="view-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:16px;padding:28px 32px;min-width:340px;max-width:420px;position:relative;">
    ...
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
      <div id="vm-av" style="width:52px;height:52px;border-radius:50%;background:#22c55e;display:flex;align-items:center;justify-content:center;font-size:1.1rem;font-weight:800;color:#fff;"></div>
      <div><div id="vm-name" style="font-weight:800;font-size:1rem;"></div><div style="font-size:.78rem;color:#6b7280;margin-top:2px;">QC Inspector</div></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      ...
      <div style="background:#f8faff;border-radius:10px;padding:12px;"><div style="font-size:.68rem;font-weight:700;color:#9ca3af;text-transform:uppercase;margin-bottom:4px;">Slot</div><div id="vm-sl" style="font-weight:700;color:#22c55e;"></div></div>
